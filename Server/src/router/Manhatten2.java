...
public class Manhatten2 extends Scenario {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<5; i++){
			...
			for(Router router : routers){
				...
				//state.printRoute();
				if(state == null){
					System.out.println("Could not route\n");
					continue;
				}
				state.printStats();
	...
	public Manhatten2(int width, int length, double chargerP, int startI, int startJ, int endI, int endJ, Amount<Length> nodeSeparation, Amount<Length> maxEdgeLength, List<Amount<Velocity>> speeds, Car vehicle){
		...
		Set<Node> chargerSet = new HashSet<Node>();
		//Build the nodes
		for(int i=0; i< width; i++){
			for(int j=0; j<length; j++){
				...
				Node c = new Node(id, id, id, coordinates, id, id, Connector.getNewCollection(chargerP, true));
				chargerSet.add(c);
				graph.addNode(c);
		...
		//Build the edges
		Random r = new Random();
		for(Node edgeStart : chargerSet){
			for(Node edgeEnd : chargerSet){
